...
class DPN(nn.Module):
    def __init__(
        ...
        # conv1
        if small:
            self.blocks["conv1_1"] = InputBlock(
        ...
        self.blocks["conv2_1"] = DualPathBlock(
            num_init_features, r, r, bw, inc, groups, "proj", b
        )
        in_chs = bw + 3 * inc
        for i in range(2, k_sec[0] + 1):
            self.blocks["conv2_" + str(i)] = DualPathBlock(
        ...
        for i in range(2, k_sec[1] + 1):
            self.blocks["conv3_" + str(i)] = DualPathBlock(
                in_chs, r, r, bw, inc, groups, "normal", b
            )
            in_chs += inc
        ...
        for i in range(2, k_sec[2] + 1):
            self.blocks["conv4_" + str(i)] = DualPathBlock(
                in_chs, r, r, bw, inc, groups, "normal", b
            )
            in_chs += inc
        ...
        for i in range(2, k_sec[3] + 1):
            self.blocks["conv5_" + str(i)] = DualPathBlock(
                in_chs, r, r, bw, inc, groups, "normal", b
            )
            in_chs += inc
