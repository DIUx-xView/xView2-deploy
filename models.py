...
class XViewFirstPlaceLocModel(nn.Module):
    ...
    def load_models(self):
        for ii, seed in enumerate([0, 1, 2]):
            snap_to_load = self.checkpoint_dict[self.model_size].replace(
                "{}", str(seed)
            )
            model = self.model_dict[self.model_size]()
            print("=> loading checkpoint '{}'".format(snap_to_load))
            checkpoint = torch.load(
                path.join(self.models_folder, snap_to_load), map_location="cpu"
            )
            loaded_dict = checkpoint["state_dict"]
            loaded_dict = {
                key.replace("module.", ""): value for key, value in loaded_dict.items()
            }
            sd = model.state_dict()
            for inner_idx, k in enumerate(model.state_dict()):
                if k in loaded_dict and sd[k].size() == loaded_dict[k].size():
                    ...
                    if inner_idx == 0:
                        print(
                            "loaded first layer"
                        )  # --> debug to make sure model loaded!
            loaded_dict = sd
    ...
    def forward(self, x, debug=False):
        ...
        for i in range(msk0.shape[0]):
            ...
            msk_out.append(
                torch.tensor(pred_full.astype("uint8").transpose(1, 2, 0)).squeeze()
            )
...
class XViewFirstPlaceClsModel(XViewFirstPlaceLocModel):
    def __init__(
        self, model_size, models_folder="weights", devices=[0, 0, 0], dp_mode=False
    ):
        super(XViewFirstPlaceClsModel, self).__init__(
            model_size,
