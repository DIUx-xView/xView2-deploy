...
class TrainData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        lbl_msk1 = cv2.imread(
            fn.replace("/images/", "/masks/").replace(
        ...
        for i in range(try_cnt):
            ...
            _sc = (
                msk2[y0 : y0 + crop_size, x0 : x0 + crop_size].sum() * 5
...
class ValData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        msk_loc = cv2.imread(
            path.join(
                loc_folder,
                "{0}.png".format(fn.split("/")[-1].replace(".png", "_part1.png")),
            ),
            cv2.IMREAD_UNCHANGED,
        ) > (0.3 * 255)
...
def validate(net, data_loader):
    ...
    sc = 0.3 * d0 + 0.7 * f1
    print(
        "Val Score: {}, Dice: {}, F1: {}, F1_0: {}, F1_1: {}, F1_2: {}, F1_3: {}".format(
...
def train_epoch(
    current_epoch, seg_loss, ce_loss, model, optimizer, scheduler, train_data_loader
):
    losses = AverageMeter()
    ...
    print(
        "epoch: {}; lr {:.7f}; Loss {loss.avg:.4f}; CCE_loss {loss1.avg:.4f}; Dice {dice.avg:.4f}".format(
            current_epoch,
            scheduler.get_lr()[-1],
            loss=losses,
            loss1=losses1,
...
if __name__ == "__main__":
    ...
    for epoch in range(2):
        train_epoch(
            epoch, seg_loss, ce_loss, model, optimizer, scheduler, train_data_loader
        )
        torch.cuda.empty_cache()
