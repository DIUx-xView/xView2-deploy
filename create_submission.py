...
from skimage.morphology import dilation, remove_small_objects, square
...
def process_image(f):
    ...
    for d in pred_folders:
        ...
        msk2 = cv2.imread(
            path.join(d, f.replace("_part1", "_part2")), cv2.IMREAD_UNCHANGED
        )
    ...
    cv2.imwrite(
        path.join(
            sub_folder,
            "{0}".format(f.replace("_part1.png", "_localization_prediction.png")),
        ),
        msk_loc,
    ...
    cv2.imwrite(
        path.join(
            sub_folder, "{0}".format(f.replace("_part1.png", "_damage_prediction.png"))
        ),
        msk_dmg,
        [cv2.IMWRITE_PNG_COMPRESSION, 9],
    )
