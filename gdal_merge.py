...
def run( out_file=None, names=None, pre_init=[255] ):
    ...
    if Driver is None:
        raise ValueError('Format driver %s not found, pick a supported driver.' % format)

    DriverMD = Driver.GetMetadata()
    if 'DCAP_CREATE' not in DriverMD:
        raise ValueError('Format driver %s does not support creation and piecewise writing.\nPlease select a format that does, such as GTiff (the default) or HFA (Erdas Imagine).' % format)
