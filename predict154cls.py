...
if __name__ == "__main__":
    ...
    post_chips = [
        os.path.join(test_dir.replace("pre", "post"), a)
        for a in os.listdir(test_dir.replace("pre", "post"))
    ]
    ...
    with torch.no_grad():
        for i in tqdm(range(len(pre_chips))):
            if True:
                ...
                cv2.imwrite(
                    path.join(
                        pred_folder, "{0}".format(rt.replace(".tif", "_part1.png"))
                    ),
                    msk[..., :3],
                ...
                cv2.imwrite(
                    path.join(
                        pred_folder, "{0}".format(rt.replace(".tif", "_part2.png"))
                    ),
                    msk[..., 2:],
                    [cv2.IMWRITE_PNG_COMPRESSION, 9],
                )
