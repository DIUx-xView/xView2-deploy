...
if __name__ == "__main__":
    ...
    for d in pred_folders:
        msk1 = cv2.imread(
            path.join(d, "{0}".format(cls_fn + "_part1.png")), cv2.IMREAD_UNCHANGED
        )
        msk2 = cv2.imread(
            path.join(d, "{0}".format(cls_fn + "_part2.png")), cv2.IMREAD_UNCHANGED
        )
    ...
    loc_preds = (
        np.asarray(loc_preds).astype("float").sum(axis=0) / len(loc_folders) / 255
    )
